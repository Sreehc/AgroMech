...
export function StructuredAnswerCard({
  ...
  return (
    <article
      className="my-3 grid gap-3 rounded-2xl border border-border bg-surface-panel/65 p-4 text-sm"
      data-structured-answer
    >
      ...
      {payload.safety_warnings.length ? (
        <Alert tone="warning">
          ...
          <AlertDescription>
            {payload.safety_warnings.join("；")}
          </AlertDescription>
        </Alert>
      ) : null}

      <section className="grid gap-2 rounded-xl border border-border/70 bg-surface-raised/85 p-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="font-medium text-foreground">不确定性</span>
          <Badge tone={uncertaintyTone(payload.uncertainty.level)}>
      ...
      {payload.visual_observation?.trim() ? (
        <StructuredSection
          title="视觉观察"
          body={payload.visual_observation.trim()}
        />
      ) : null}
      ...
      {payload.ocr_text?.trim() ? (
        <StructuredSection title="OCR" body={payload.ocr_text.trim()} />
      ) : null}
      ...
      <section className="grid gap-2 rounded-xl border border-border/70 bg-surface-raised/85 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          {payload.trace_id ? (
            <Badge tone="neutral">Trace {payload.trace_id}</Badge>
          ) : null}
        </div>
...
function StructuredSection({ title, body }: { title: string; body: string }) {
  return (
    <section className="grid gap-1 rounded-xl border border-border/70 bg-surface-raised/85 p-3">
...
function CitationEntry({
  ...
  return (
    <div className="grid gap-2 rounded-xl border border-border/70 bg-surface-raised/85 p-3">
