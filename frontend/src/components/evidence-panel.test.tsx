...
describe("EvidencePanel", () => {
  it("renders selected citation details, trace, uncertainty, safety warning and document link", () => {
    const html = renderToStaticMarkup(<EvidencePanel payload={payload()} activeIndex={0} onSelect={() => {}} />);
  ...
  it("renders an accessible close control when the workbench opens the panel", () => {
    const html = renderToStaticMarkup(<EvidencePanel payload={payload()} activeIndex={0} onClose={() => {}} />);
  ...
  it("shows inaccessible source state and keeps citation metadata", () => {
    const html = renderToStaticMarkup(<EvidencePanel payload={payload()} activeIndex={1} onSelect={() => {}} />);
  ...
  it("keeps mobile sheet and close wiring in the workbench evidence region", () => {
    const workbench = readFileSync(new URL("./assistant-workbench.tsx", import.meta.url), "utf8");

    expect(workbench).toContain("max-xl:fixed");
    expect(workbench).toContain("max-xl:bottom-3");
    expect(workbench).toContain("onClose={selectedEvidence ? onEvidenceClose : undefined}");
  });

  it("renders a stable empty state when no citations are available", () => {
    const html = renderToStaticMarkup(<EvidencePanel payload={payload({ citations: [] })} activeIndex={0} />);
