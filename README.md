...
- 图谱：Neo4j，复用 `../infrastructure` 中的共享实例。
...
启动共享本地依赖服务。Postgres 和 Neo4j 不在本项目中新建 Docker 环境，统一使用同级目录 `../infrastructure`：

```bash
cd ../infrastructure
docker compose --env-file env/.env -f compose/docker-compose.core.yml up -d postgres
docker compose --env-file env/.env -f compose/docker-compose.graph.yml up -d neo4j
cd ../AgroMech
```
...
- 无图片附件时，Next API route 调用 FastAPI `POST /qa/text`。
- 带单张图片附件时，Next API route 调用 FastAPI `POST /qa/image`。
- 回复会展示回答正文、视觉观察、引用来源、安全提醒和 trace 信息。

Next server-side adapter 默认读取：

```bash
AGROMECH_API_BASE_URL=http://127.0.0.1:8000
AGROMECH_ADMIN_USERNAME=admin
AGROMECH_ADMIN_PASSWORD=change-me
```
