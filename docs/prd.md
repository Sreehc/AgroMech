...
- 支持文本、表格、PDF 页面渲染、图片 OCR、视觉观察、LLM 元数据回填、实体抽取、全文索引和 pgvector 向量索引；Graph RAG 暂不在主链路启用。
...
- 当前 Agentic RAG 为工程可控版本：规则优先路由、证据检查、query rewrite、最多 2 轮补检索、生成前 evidence guard。

### 检索和证据

- 混合检索包含关键词、结构化过滤、pgvector 向量检索、Vision RAG 和 rerank；Graph RAG 暂不在当前验收范围。
- 结果按 chunk 去重，保留命中通道和降级信息。
- 最终回答必须带 citation；证据不足时返回高不确定性回答。
